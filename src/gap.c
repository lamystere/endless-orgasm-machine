...
static void print_conn_desc(struct ble_gap_conn_desc *desc) {
    ...
    /* Connection handle */
    ESP_LOGD(TAG, "connection handle: %d", desc->conn_handle);

    /* Local ID address */
    format_addr(addr_str, desc->our_id_addr.val);
    ESP_LOGD(TAG, "device id address: type=%d, value=%s",
             desc->our_id_addr.type, addr_str);

    /* Peer ID address */
    format_addr(addr_str, desc->peer_id_addr.val);
    ESP_LOGD(TAG, "peer id address: type=%d, value=%s", desc->peer_id_addr.type,
             addr_str);
...
void adv_init(void) {
    ...
    ESP_LOGD(TAG, "device address: %s", addr_str);
